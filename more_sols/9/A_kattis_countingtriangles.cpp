...
int main()
{
    ...
    while (cin >> n)
    {
        if (!n)
            break;
        vector<vector<point>> v(n, vector<point>(2));
        for (int i = 0; i < n; i++)
        {
            double x1, y1, x2, y2;
            cin >> x1 >> y1 >> x2 >> y2;
        ...
        auto onSegment = [](point p1, point p2, point t)
        {
            return (min(p1.x, p2.x) <= t.x && t.x <= max(p1.x, p2.x) && min(p1.y, p2.y) <= t.y && t.y <= max(p1.y, p2.y));
        };
        int res = 0;
        for (int i = 0; i < n; i++)
        {
            point a1 = v[i][0], a2 = v[i][1];
            line l1;
            pointsToLine(a1, a2, l1);
            for (int j = i + 1; j < n; j++)
            {
                point b1 = v[j][0], b2 = v[j][1];
                line l2;
                pointsToLine(b1, b2, l2);
                point t;
                if (areIntersect(l1, l2, t) && onSegment(a1, a2, t) && onSegment(b1, b2, t))
                {
                    for (int k = j + 1; k < n; k++)
                    {
                        point c1 = v[k][0], c2 = v[k][1];
                        line l3;
                        pointsToLine(c1, c2, l3);
                        point t1, t2;
                        if (areIntersect(l1, l3, t1) && onSegment(a1, a2, t1) && onSegment(c1, c2, t1) && areIntersect(l3, l2, t2) && onSegment(c1, c2, t2) && onSegment(b1, b2, t2))
                        {
                            res++;
                        }
                    }
