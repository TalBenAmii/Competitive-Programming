...
struct point
{
    ...
    point(const point_i &p) : x(p.x), y(p.y) {}
...
//// another variant
int area2(point_i p, point_i q, point_i r)
...
int main()
{
    ...
    while (N > 0)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            int x, y;
            cin >> x >> y;
            vp.push_back(point_i(x, y));
        }
        int i = 1;
        bool is_ccw = ccw(vp[N - 1], vp[0], vp[1]) > 0;
        int flag = true;
        for (; i < N; i++)
        {
            if (ccw(vp[(i - 1)], vp[i], vp[(i + 1) % N]) != is_ccw)
            {
                flag = false;
                break;
            }
